...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\columnwidth, height=0.43\textheight, keepaspectratio]{f2_arms_by_model-newer.jpg}
...
Every reasoning-capable model spends more thinking tokens on a story than on the matched literal description of the same statement: between $1.1\times$ and $1.5\times$ as many at the median, with the strongest reasoners at the high end of that range (Appendix~\ref{app:tokens}, Table~\ref{tab:tokens}). The model has to spend additional inference-time compute to extract the formal structure from the unstructured narrative before it can translate it. 
% \shivam{This result also looks trivial, one would expect more token usage for the reasoning model, and more complex task, it should be moved to appendix in favor ouf other results.} \denis{done}
