...
% \usepackage[final]{neurips_2026_vericode}
...
\newtcolorbox{promptbox}{breakable,ignore nobreak=true,enhanced,colback=white,colframe=black,boxrule=0.4pt,arc=0pt,left=4pt,right=4pt,top=4pt,bottom=4pt}
...
\author{%
  Oren E. Hartstein\thanks{Equal contribution.} \\
  Columbia University \\
  \texttt{oeh2104@columbia.edu} \\
  \And
  Denis F. Lim\footnotemark[1] \\
  Cambridge AI Safety Hub \\
  \texttt{denislim95@gmail.com} \\
  \And
  Harsh Soni \\
  UC Berkeley \\
  \texttt{harsh.soni@berkeley.edu} \\
  \AND
  Ke Zhang \\
  UC Irvine \\
  \texttt{kzhan153@ucr.edu} \\
  \And
  Luiza Corpaci \\
  Independent \\
  \texttt{corpaci.luiza@gmail.com} \\
  \And
  Shivam Raval \\
  Harvard \\
  \texttt{sraval@g.harvard.edu} \\
}
...
\subsection{Reasoning significantly improves translation accuracy across complexity}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.65\columnwidth]{tokens_vs_complexity-new.jpg}
